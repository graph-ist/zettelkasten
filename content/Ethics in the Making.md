...
flowchart TD
    ...
    Phenomenology --> MerleauPonty[Merleau-Ponty]
    MerleauPonty --> EmbodiedInteraction[Embodied Interaction]
    Vygotsky --> SituatedLearning[Situated Learning]
    SituatedLearning --> Lave[Lave & Wenger]
    Latour --> ANT[Actor-Network Theory]
    ANT --> Jonsson[Jönsson]
    Schon[Schön] --> ReflectivePractice[Reflective Practice]
    ReflectivePractice --> Jonsson
    Norman --> Affordance
    Affordance --> Jonsson
    Dourish --> Jonsson
    EmbodiedInteraction --> Jonsson
    Gaver --> CulturalProbes[Cultural Probes]
    CulturalProbes --> Jonsson

    class Husserl,Phenomenology,MerleauPonty,EmbodiedInteraction,Vygotsky,SituatedLearning,Lave,Latour,ANT,Schon,ReflectivePractice,Norman,Affordance,Dourish,Gaver,CulturalProbes,Jonsson internal-link;
```
